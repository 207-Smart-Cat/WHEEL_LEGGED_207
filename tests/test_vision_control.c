...
int main(void)
{
    ...
    assert_close(vision_control_update(&state, 10), -1.645f);

    vision_control_reset(&state);
    assert_close(vision_control_update(&state, 100), -11.2f);

    vision_control_reset(&state);
    state.filtered_offset_deg = 17.0f;
    assert_close(vision_control_update(&state, 100), -0.15f);

    vision_control_reset(&state);
    assert_close(vision_control_update(&state, 1000), -112.0f);

    vision_control_reset(&state);
    assert_close(vision_control_update(&state, 50), -5.6f);
    assert_close(vision_control_update(&state, 30), -5.95f);
