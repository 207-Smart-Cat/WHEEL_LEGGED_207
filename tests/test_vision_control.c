...
static void test_yaw_rate_direction_and_damping(void)
{
    ...
    assert(vision_turn_control_update(&state, 0.0f, vision_yaw_rate,
                                      COURSE3_VISION_CONTROL_DT_S, 1U) < 0.0f);
    assert(state.d_output < 0.0f);
...
static void test_conditional_integral(void)
{
    ...
    assert_close(vision_turn_control_update(&state, 3.0f, 0.0f,
                                            COURSE3_VISION_CONTROL_DT_S, 1U), 60.0f);
    ...
    assert_close(state.i_output, 12.0f);
    ...
    assert_close(state.integral_deg_s, 0.0f);
    assert_close(state.output, 1.5f);
