 ...
 * - 工程适配接口负责当前项目的 5 ms 调度、数据采集和低频日志。
...
#define BUMPY_WINDOW_MAX_SAMPLES    (64U)   /* 单个滑动窗口允许的最大采样数。 */
...
/*
 * 当前工程的单模块适配接口。
 * Process_5ms 只需在后续确认的 5ms 调度点调用；Log_Task 必须在主循环调用，
 * 不能从中断打印。当前版本默认 OBSERVE，不会覆盖速度、航向或腿部输出。
 */
void Bumpy_Project_Init(void);
void Bumpy_Project_Process_5ms(void);      /* 5 ms 调度调用。 */
