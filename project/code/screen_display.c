...
static const ui_cn_glyph_t k_ui_cn_glyphs[] = {
    ...
    {0x901F, {0x00,0x40,0x27,0xFE,0x37,0xFE,0x10,0x40,0x03,0xFC,0x02,0x44,0x72,0x44,0x37,0xFC,0x30,0xD0,0x31,0x58,0x32,0x46,0x34,0x40,0x38,0x00,0xC7,0xFE,0x40,0x00,0x00,0x00}},
    {0x9AD8, {0x01,0x00,0x01,0x80,0x7F,0xFE,0x1F,0xF0,0x08,0x10,0x08,0x10,0x1F,0xF0,0x7F,0xFE,0x20,0x06,0x27,0xE6,0x24,0x26,0x24,0x26,0x27,0xE6,0x20,0x0E,0x00,0x08,0x00,0x00}},
...
static const uint16_t UI_TEXT_T_TYPE_HIGH_SPEED[] = {0x9AD8, 0x901F, 0x0000};
...
static void ui_set_screen(ui_screen_t screen)
{
    ...
    if (screen == UI_SCREEN_VISION)
    {
        ...
        if (!ui_course3_vision_active)
        {
            ui_send_param_update(P_TARGET_VELOCITY, VISION_MENU_CAL_SPEED);
        }
        IPC_Update_Vision_Command_CoreB(1U, 0U, 0U, 0, 0.0f);
    }
    else if (ui_screen == UI_SCREEN_VISION)
    {
        (void)CameraAssist_FlushExposureConfig();
...
static const uint16_t *ui_waypoint_type_text(uint8 type)
{
    switch ((WayPoint_Type)type)
    {
        ...
        case WP_TYPE_HIGH_SPEED: return UI_TEXT_T_TYPE_HIGH_SPEED;
        case WP_TYPE_NORMAL:
...
static void ui_handle_bump(ui_key_event_t events[UI_KEY_COUNT])
{
    ...
    else if (events[UI_KEY_OK])
    {
        ...
        {
            IPC_Set_Manual_Test_Mode(MANUAL_TEST_MODE_BUMP);
            IPC_Bump_Set_Run(IPC_Bump_Get_Run() ? 0U : 1U);
        }
