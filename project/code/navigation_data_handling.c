...
void navi_parse_data(void) {
  ...
  // 扣除初始偏移量，计算相对航向角     ,并归一化
   ...
   // --- A. 速度处理：低通滤波 ---  
    ...
    filter_data.unbiased_gyro[2] = low_pass_filter_update(&lpf_w, raw_data.unbiased_gyro[2] - 0.1);           //！！！这里0.1是静置时陀螺仪的小零偏
...
void navi_ekf_update(void) {
    ...
    // 如果编码器速度极小，且角速度变化也不大（说明真停住了，不是在腾空打滑）
    if (fabsf(v_obs_mps) < 0.01f && fabsf(filter_data.unbiased_gyro[2]) < 0.1f && !airborne_flag) {
        a_input = 0.0f;  // 暴力切断：静止时物理上不可能有加速度，直接掐死假数据输入
        
        // 技巧：在静止时临时强行给 R 赋极小值，强制将误差压进 bias_ax 里
