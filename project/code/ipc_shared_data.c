...
void IPC_Check_And_Apply_Params_To_Core0(void) {
    ...
    Bump_Control_Set_Manual_Mode(
        (core_b_cmd.manual_test_mode == (uint8)MANUAL_TEST_MODE_BUMP) ? 1U : 0U,
        core_b_cmd.bump_run ? 1U : 0U,
        bump_config.target_speed,
        bump_config.pwm_gain);
...
void IPC_Bump_Load_Config_From_Flash(void)
{
    ...
    if (!BumpMode_LoadRecord(&record, &config))
    {
        config.pwm_gain = BUMP_ACTIVE_PWM_GAIN;
