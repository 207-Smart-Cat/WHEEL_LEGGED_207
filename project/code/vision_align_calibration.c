                           ...
                           float current_yaw)
{
    ...
    if (cal->state == VISION_ALIGN_CAL_SAMPLING)
    {
        ...
        if (lane_error_px < 0)
        {
            if (cal->left_sample_count >= VISION_ALIGN_SIDE_SAMPLE_TARGET)
            {
                return;
            }
        ...
        {
            if (cal->right_sample_count >= VISION_ALIGN_SIDE_SAMPLE_TARGET)
            {
                return;
            }
            cal->right_sample_count++;
        }
        else if (cal->left_sample_count <= cal->right_sample_count)
        {
            if (cal->left_sample_count >= VISION_ALIGN_SIDE_SAMPLE_TARGET)
            {
                return;
        ...
        if (cal->sample_count >= VISION_ALIGN_SAMPLE_COUNT_TARGET)
        {
            cal->result_yaw = vision_align_wrap_angle(cal->anchor_yaw +
                cal->sample_delta_sum / (float)VISION_ALIGN_SAMPLE_COUNT_TARGET);
            cal->result_valid = 1U;
