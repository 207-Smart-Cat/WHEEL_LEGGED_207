...
void getJointAngles(float x_target, float y_target, float *phi1, float *phi4) {
    float a = 2 * (x_target + L5 / 2) * L1;
        ...
        if (sum_val >= 0) {
            float phi1_rad = 2 * atan((b + sqrt(sum_val)) / (a + c)); // �?取�?�解
            *phi1 = phi1_rad * (180.0 / PI); // 弧度�?角度
        ...
        // 计算�?二个关节角度 phi4
        float a1 = 2 * (x_target - L5 / 2) * L4; // 这里应�?�是 L4 而不�? L1
        ...
        if (sum_val >= 0) {
            float phi4_rad = 2 * atan((b1 - sqrt(sum_val)) / (a1 + c1)); // �?取负�?
            *phi4 = phi4_rad * (180.0 / PI); // 弧度�?角度
